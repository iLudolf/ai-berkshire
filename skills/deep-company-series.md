...
Write an 8-article deep-dive series for $ARGUMENTS, intended for public publication on platforms such as investment newsletters, Substack, or social media. **The core IP is not "writing well" — it's "revising rigorously" — 99% of financial articles violate the fact-checking standards in this skill**.
...
| 06 | Deconstructing X's Financials the Buffett Way | Financial deep-dive: gross margin / FCF / ROE / dividend yield | 8,000–10,000 |
...
- **Mobile-compatible opening** — the first 18–20 characters must stand on their own (mobile preview)
...
- Use **contrasting numbers** or **contrarian conclusions** as hooks (e.g., "15 Years, 7 Challenges, All Failed", "Annual Salary of R$25M Equals 0.003% of Net Income")
- Subtitles should be neutral and summarize the content ("— `<core judgment>`")
- **Avoid clickbait comparisons**: "the little Buffett", "Brazil's version of X", "GOAT" — all off-limits
...
2. **Third-party market share estimates**: Data from Economatica, Refinitiv, ANBIMA, and similar providers vary enormously by methodology (the same time point can differ 2–3×). **Only use the two most credible sources as anchors; treat others as qualitative descriptions**.
...
4. **Undisclosed ownership stakes**: Ownership stakes in unlisted companies **have never been publicly disclosed**. **Give a range, label as "unknowable"**.
...
```
□ 1. Cross-article number consistency: total market cap, net income (IFRS / adjusted), key ownership % — aligned across all 8 articles
□ 2. Accounting basis labeling: IFRS / adjusted EBITDA / FCF / GAAP — clarify which is used throughout
...
□ 4. Fairness in peer comparison: do not use "core business P/L (ex-cash + portfolio)" vs "peer P/L (not ex)"
...
□ 7. Third-party data sourcing: every non-filing data point followed by "(Source: X)"
```

### Known Hard-Error Risks

List **known hard-error risks** before writing:
- Historical return multiples: must use cumulative-cost basis
- Ownership stakes: must check latest CVM filings (FRE / DFP) and RI disclosures for current figures
- Dividend accounting (JCP — Juros sobre Capital Próprio): treated differently from standard dividends for tax purposes; label explicitly and do not conflate with dividends
- Total share count can rebound: stock option / restricted share grants concentrated at year-start can temporarily increase share count
- FII distributions: distinguish between income distributions (rendimentos) and amortization of capital (amortização) — only the former is tax-exempt for individual investors
...
1. Read the company's DFP (annual) and latest ITR (quarterly) filings on CVM; for FIIs read the Informe Mensal and Relatório Gerencial
2. Read at least 3 independent sell-side research reports (XP Investimentos, BTG Pactual Research, Itaú BBA, Goldman Sachs Brasil, UBS BB) — identify consensus + contrarian views
...
2. Whether the same terms (EBITDA, FCF, IFRS, P/L, P/VPA, DY) are explained upon first appearance
...
grep -r "israelludolf\|/Users/\|<user company alias>" reports/ | head
```
...
- Check DFP / ITR / FRE filings and financial reports for the same company in the ai-berkshire project
- Check Fundamentus, Status Invest, or CVM official disclosures
...
| Subjectivity | Strong subjective language, absolutism, clickbait comparisons | Soften or remove |
...
- Changed total market cap → update linked P/L / core business valuation / discount / FCF Yield across the entire series
...
- All public reports **use only public information** (DFP/ITR/FRE filings, CVM official disclosures, sell-side research reports, reputable third-party institutions such as Fundamentus, Status Invest, ANBIMA, B3)
- Do not use any **user personal information** (company aliases, internal conversations, undisclosed position information)
- Before pushing, must run grep to scan for username / `/Users/` / user company aliases and other private fields
